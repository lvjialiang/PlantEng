...
{
    public class ProductPrize2013Controller : Controller
    {
        ...
        public ActionResult Ajax(string action, int catId, int productId)
        {
            ...
            if (action.ToLower() == "vote")
            {
                ...
                c = PlantEng.Web.WWW.Areas.Special.Data.ProductPrizeData.VotedCount(catId, currentContext.UserId, 2013);
                if (c > 2)
                {
                    return Json(new { Status = -2 });
                }
                //默认2012年产品奖
                c = PlantEng.Web.WWW.Areas.Special.Data.ProductPrizeData.Vote(catId, productId, currentContext.UserId, 2013);
                if (c == -1)
                {
